...
test('sell_all / sell_half も保有量を減らす(sellのみ処理する旧バグの回帰テスト)', () => {
  const events: TradeEvent[] = [
    // 100コイン@10で購入 → sell_half で50コイン売却 → sell_all で残り全量売却
    { t: 1, type: 'order', dryRun: false, pair: 'xrp_jpy', action: 'buy', sizeJpy: 1000, price: 10 },
    { t: 2, type: 'order', dryRun: false, pair: 'xrp_jpy', action: 'sell_half', sizeCoin: 50, price: 12 },
    { t: 3, type: 'order', dryRun: false, pair: 'xrp_jpy', action: 'sell_all', sizeCoin: 50, price: 11 },
    // 全量売却後に買い直し → 平均取得単価は買い直し価格だけで決まる(旧実装では過去の原価が混入していた)
    { t: 4, type: 'order', dryRun: false, pair: 'xrp_jpy', action: 'buy', sizeJpy: 2000, price: 20 },
  ];
  const xrp = computeCostBasis(events).get('xrp');
  assert.ok(xrp);
  assert.equal(Math.round(xrp.amount), 100); // 2000円 ÷ 20円 = 100コインのみ
  assert.equal(xrp.avgPrice, 20); // 旧実装では (1000+2000)/(100+100)=15 に汚染されていた
});
